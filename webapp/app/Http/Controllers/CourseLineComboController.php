...
class CourseLineComboController extends Controller
{
    public function index(Request $request, string $course_key)
    {
        ...
        $sort           = $request->query('sort', 'start_count');
        ...
        |------------------------------------------------------------------
        */
        $yearOptionsQuery = DB::table('ri_course_line_combo')
        ...
        |------------------------------------------------------------------
        */
        $query = DB::table('ri_course_line_combo')
            ->where('course_key', $course_key)
            ->where('grade_group', $grade);
        ...
        $rows = $query
            ...
            ->map(function ($r) {
                ...
                $r->combo_label = sprintf(
                    '%s × %s',
                    $r->father_line_key_detail ?? '(父不明)',
                    $r->mf_line_key_detail ?? '(母父不明)'
                );
                return $r;
            })
    ...
    {
        $allowed = [
            'start_count','win_rate','place_rate',
            'show_rate','board_rate',
            'win_count','place_count',
            'show_count','board_count','out_of_board_count',
        ];

        return in_array($sort, $allowed, true) ? $sort : 'start_count';
    }
}
